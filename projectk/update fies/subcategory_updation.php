...
$sql = "SELECT * FROM subcategories LIMIT $limit OFFSET $offset";
...
if ($result->num_rows > 0) {
    ...
    <input type="text" name="subcategory_id" value="<?php echo $row['subcategory_id']; ?>">
    <label for="subcategory_name">Subcategory Name:</label>
    <input type="text" name="subcategory_name" value="<?php echo $row['subcategory_name']; ?>">
...
$sql = "SELECT COUNT(*) AS total_records FROM subcategories";
$result = $conn->query($sql);
...
if ($total_pages > 1) {
    echo "<div>";
    if ($current_page > 1) {
        echo "<a href='subcategory_updation.php?page=" . ($current_page - 1) . "'>Prev</a> ";
    }
    for ($i = 1; $i <= $total_pages; $i++) {
        if ($i == $current_page) {
            echo "<strong>$i</strong> ";
            } else {
            echo "<a href='subcategory_updation.php?page=$i'>$i</a> ";
            }
            }
            if ($current_page < $total_pages) {
            echo "<a href='subcategory_updation.php?page=" . ($current_page + 1) . "'>Next</a>";
            }
            echo "</div>";
            }
