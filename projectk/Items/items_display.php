...
<body>
  ...
  <form method="GET" action="/projectk/items/items_display.php?subcategory_id=<?php echo $_GET['subcategory_id']; ?>">
  ...
  $sql = "SELECT *
        ...
        OR item_id LIKE '%$search_value%'
        OR place_stored LIKE '%$search_value%'
        OR details LIKE '%$search_value%'
        OR current_stock LIKE '%$search_value%')";
...
if (mysqli_num_rows($result) > 0) {
  ...
  ?>
      <div class='container'>
        <img src='uploads/<?php echo $row["image"] ?>' alt='<?php echo $row["item_name"] ?>'><br>
...
while($row = $result_stock->fetch_assoc()) {
  echo "<tr><td>" . $row["username"] . "</td>
        ...
        <td>" . $row["price"] . "</td>
        <td>" . $row["quantity"] . "</td>
        <td><a href='invoice/" .$row["invoice"] . "' target='_blank' rel='noopener noreferrer'><button>View Invoice</button></a></td></tr>";
}
